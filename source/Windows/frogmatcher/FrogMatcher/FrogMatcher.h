  ...
  public:
				...
				if (outMatches.size()>0) {
					...
					for (std::vector<cv::DMatch>::const_iterator it= outMatches.begin();
							it!= outMatches.end(); ++it) {
								...
								double dist = sqrtl(long double((x-x2)*(x-x2) + (y-y2)*(y-y2)));
		...
		// ##########################################################################################
          cv::Mat match(cv::Mat& image1, cv::Mat& image2, // input images 
				...
				f_err = 0;
				thresholdDist = 0.20 * sqrtl(long double(image1.size().width*image1.size().width + image1.size().height*image1.size().height));
				thresholdDist2 = 0.3 * sqrtl(long double(image2.size().width*image2.size().width + image2.size().height*image2.size().height));
